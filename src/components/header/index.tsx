    ...
    return (
        <header className="flex w-screen flex-col h-24 justify-center">
            ...
            <div className="flex justify-center gap-2">
                <input onChange={(e) => setUsername(e.target.value)} value={username} className={`w-[60%] h-10 rounded-xl p-4 md:outline-none ${themeIsDark ? 'bg-slate-700 text-white' : 'bg-gray-300'}`} type="text" />
                <button onClick={() => getData(username)} className="bg-blue-500 px-2 rounded-lg text-white">Search</button>
            </div>  
        </header>
    )
}